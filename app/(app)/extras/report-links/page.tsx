...
export default async function ReportLinksPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const { data: reports } = selectedClientId
    ? await sb
        .from('shared_reports')
        .select('id, client_id, year_month, title, share_token, created_at')
        .eq('client_id', selectedClientId)
        .order('year_month', { ascending: false })
    : { data: [] as Array<{ id: number; client_id: number; year_month: string; title: string; share_token: string; created_at: string }> };
      ...
      <section className="bg-white rounded-3xl shadow p-6 space-y-5">
        <form className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_200px_auto] md:items-end">
          <div>
            ...
            <select name="client" defaultValue={selectedClientId ?? ''} className="w-full border border-gray-300 rounded-xl p-3">
              {clients?.map((item) => (
        ...
        <div className="rounded-3xl border border-gray-200 bg-[#fafaf8] p-5">
          ...
          <div className="mt-3 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">{selectedClient?.company_name ?? '-'}</p>
            </div>
          ...
          {reports?.map((report) => (
            <div key={report.id} className="flex flex-col gap-4 px-6 py-5 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">{report.year_month}</p>
              </div>
